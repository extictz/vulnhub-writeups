# 🎯 Informe de Vulneración de la Máquina "DEVGURU1"
...
Identificamos la IP de la víctima, asignada por la red NAT: 10.0.2.7. Los puertos abiertos son 22 (SSH) y 80 (HTTP).

Vamos a realizar un escaneo más profundo de la IP de la víctima.

Encontramos un enlace que nos proporciona una URL: devguru.local.
...
En el archivo hosts, asignamos la IP al dominio para que sea reconocido por nuestra máquina.
...
# Clonación de GitTools 📂
Clonamos GitTools para probar la existencia de directorios en la URL.
...
Ingresamos en el archivo website y listamos los archivos generados.

Nos interesa adminer.php. Accedemos al directorio desde la web.

Vemos un panel de inicio de sesión. Examinamos el código fuente en busca de información.
...
Entramos al archivo config del EXTRACTOR y analizamos database.php.

Obtenemos información para acceder a la base de datos MySQL.
...
# Acceso al Panel de Administración 🛠️
Accedemos al backend con las nuevas credenciales.
...
Tenemos un exitoso acceso a la página de october:
...
Buscamos un exploit para sqlite y ejecutamos un comando que nos otorga acceso de superusuario.

Verificamos el acceso a la raíz del sistema.
...
Hemos conseguido acceso completo al sistema y obtenido la flag de root.

Espero que te haya sido de ayuda <3
